/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import { Signer, utils, Contract, ContractFactory, Overrides } from "ethers";
import { Provider, TransactionRequest } from "@ethersproject/providers";
import type {
  YVaultV2PriceOracle,
  YVaultV2PriceOracleInterface,
} from "../YVaultV2PriceOracle";

const _abi = [
  {
    inputs: [
      {
        internalType: "contract CToken",
        name: "cToken",
        type: "address",
      },
    ],
    name: "getUnderlyingPrice",
    outputs: [
      {
        internalType: "uint256",
        name: "",
        type: "uint256",
      },
    ],
    stateMutability: "view",
    type: "function",
  },
];

const _bytecode =
  "";

type YVaultV2PriceOracleConstructorParams =
  | [signer?: Signer]
  | ConstructorParameters<typeof ContractFactory>;

const isSuperArgs = (
  xs: YVaultV2PriceOracleConstructorParams
): xs is ConstructorParameters<typeof ContractFactory> => xs.length > 1;

export class YVaultV2PriceOracle__factory extends ContractFactory {
  constructor(...args: YVaultV2PriceOracleConstructorParams) {
    if (isSuperArgs(args)) {
      super(...args);
    } else {
      super(_abi, _bytecode, args[0]);
    }
    this.contractName = "YVaultV2PriceOracle";
  }

  deploy(
    overrides?: Overrides & { from?: string | Promise<string> }
  ): Promise<YVaultV2PriceOracle> {
    return super.deploy(overrides || {}) as Promise<YVaultV2PriceOracle>;
  }
  getDeployTransaction(
    overrides?: Overrides & { from?: string | Promise<string> }
  ): TransactionRequest {
    return super.getDeployTransaction(overrides || {});
  }
  attach(address: string): YVaultV2PriceOracle {
    return super.attach(address) as YVaultV2PriceOracle;
  }
  connect(signer: Signer): YVaultV2PriceOracle__factory {
    return super.connect(signer) as YVaultV2PriceOracle__factory;
  }
  static readonly contractName: "YVaultV2PriceOracle";
  public readonly contractName: "YVaultV2PriceOracle";
  static readonly bytecode = _bytecode;
  static readonly abi = _abi;
  static createInterface(): YVaultV2PriceOracleInterface {
    return new utils.Interface(_abi) as YVaultV2PriceOracleInterface;
  }
  static connect(
    address: string,
    signerOrProvider: Signer | Provider
  ): YVaultV2PriceOracle {
    return new Contract(address, _abi, signerOrProvider) as YVaultV2PriceOracle;
  }
}
