/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import { Signer, utils, Contract, ContractFactory, Overrides } from "ethers";
import { Provider, TransactionRequest } from "@ethersproject/providers";
import type {
  UniswapTwapPriceOracleV2Factory,
  UniswapTwapPriceOracleV2FactoryInterface,
} from "../UniswapTwapPriceOracleV2Factory";

const _abi = [
  {
    inputs: [
      {
        internalType: "address",
        name: "_rootOracle",
        type: "address",
      },
      {
        internalType: "address",
        name: "_logic",
        type: "address",
      },
    ],
    stateMutability: "nonpayable",
    type: "constructor",
  },
  {
    inputs: [],
    name: "WETH",
    outputs: [
      {
        internalType: "address",
        name: "",
        type: "address",
      },
    ],
    stateMutability: "view",
    type: "function",
    constant: true,
  },
  {
    inputs: [],
    name: "logic",
    outputs: [
      {
        internalType: "address",
        name: "",
        type: "address",
      },
    ],
    stateMutability: "view",
    type: "function",
    constant: true,
  },
  {
    inputs: [
      {
        internalType: "address",
        name: "",
        type: "address",
      },
      {
        internalType: "address",
        name: "",
        type: "address",
      },
    ],
    name: "oracles",
    outputs: [
      {
        internalType: "contract UniswapTwapPriceOracleV2",
        name: "",
        type: "address",
      },
    ],
    stateMutability: "view",
    type: "function",
    constant: true,
  },
  {
    inputs: [],
    name: "rootOracle",
    outputs: [
      {
        internalType: "address",
        name: "",
        type: "address",
      },
    ],
    stateMutability: "view",
    type: "function",
    constant: true,
  },
  {
    inputs: [
      {
        internalType: "address",
        name: "uniswapV2Factory",
        type: "address",
      },
      {
        internalType: "address",
        name: "baseToken",
        type: "address",
      },
    ],
    name: "deploy",
    outputs: [
      {
        internalType: "address",
        name: "",
        type: "address",
      },
    ],
    stateMutability: "nonpayable",
    type: "function",
  },
];

const _bytecode =
  "";

type UniswapTwapPriceOracleV2FactoryConstructorParams =
  | [signer?: Signer]
  | ConstructorParameters<typeof ContractFactory>;

const isSuperArgs = (
  xs: UniswapTwapPriceOracleV2FactoryConstructorParams
): xs is ConstructorParameters<typeof ContractFactory> => xs.length > 1;

export class UniswapTwapPriceOracleV2Factory__factory extends ContractFactory {
  constructor(...args: UniswapTwapPriceOracleV2FactoryConstructorParams) {
    if (isSuperArgs(args)) {
      super(...args);
    } else {
      super(_abi, _bytecode, args[0]);
    }
    this.contractName = "UniswapTwapPriceOracleV2Factory";
  }

  deploy(
    _rootOracle: string,
    _logic: string,
    overrides?: Overrides & { from?: string | Promise<string> }
  ): Promise<UniswapTwapPriceOracleV2Factory> {
    return super.deploy(
      _rootOracle,
      _logic,
      overrides || {}
    ) as Promise<UniswapTwapPriceOracleV2Factory>;
  }
  getDeployTransaction(
    _rootOracle: string,
    _logic: string,
    overrides?: Overrides & { from?: string | Promise<string> }
  ): TransactionRequest {
    return super.getDeployTransaction(_rootOracle, _logic, overrides || {});
  }
  attach(address: string): UniswapTwapPriceOracleV2Factory {
    return super.attach(address) as UniswapTwapPriceOracleV2Factory;
  }
  connect(signer: Signer): UniswapTwapPriceOracleV2Factory__factory {
    return super.connect(signer) as UniswapTwapPriceOracleV2Factory__factory;
  }
  static readonly contractName: "UniswapTwapPriceOracleV2Factory";
  public readonly contractName: "UniswapTwapPriceOracleV2Factory";
  static readonly bytecode = _bytecode;
  static readonly abi = _abi;
  static createInterface(): UniswapTwapPriceOracleV2FactoryInterface {
    return new utils.Interface(
      _abi
    ) as UniswapTwapPriceOracleV2FactoryInterface;
  }
  static connect(
    address: string,
    signerOrProvider: Signer | Provider
  ): UniswapTwapPriceOracleV2Factory {
    return new Contract(
      address,
      _abi,
      signerOrProvider
    ) as UniswapTwapPriceOracleV2Factory;
  }
}
