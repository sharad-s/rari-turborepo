/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import { Signer, utils, Contract, ContractFactory, Overrides } from "ethers";
import { Provider, TransactionRequest } from "@ethersproject/providers";
import type {
  UniswapV3TwapPriceOracleV2Factory,
  UniswapV3TwapPriceOracleV2FactoryInterface,
} from "../UniswapV3TwapPriceOracleV2Factory";

const _abi = [
  {
    inputs: [
      {
        internalType: "address",
        name: "_logic",
        type: "address",
      },
    ],
    stateMutability: "nonpayable",
    type: "constructor",
  },
  {
    inputs: [],
    name: "WETH",
    outputs: [
      {
        internalType: "address",
        name: "",
        type: "address",
      },
    ],
    stateMutability: "view",
    type: "function",
    constant: true,
  },
  {
    inputs: [],
    name: "logic",
    outputs: [
      {
        internalType: "address",
        name: "",
        type: "address",
      },
    ],
    stateMutability: "view",
    type: "function",
    constant: true,
  },
  {
    inputs: [
      {
        internalType: "address",
        name: "",
        type: "address",
      },
      {
        internalType: "uint256",
        name: "",
        type: "uint256",
      },
      {
        internalType: "address",
        name: "",
        type: "address",
      },
    ],
    name: "oracles",
    outputs: [
      {
        internalType: "contract UniswapV3TwapPriceOracleV2",
        name: "",
        type: "address",
      },
    ],
    stateMutability: "view",
    type: "function",
    constant: true,
  },
  {
    inputs: [
      {
        internalType: "address",
        name: "uniswapV3Factory",
        type: "address",
      },
      {
        internalType: "uint24",
        name: "feeTier",
        type: "uint24",
      },
      {
        internalType: "address",
        name: "baseToken",
        type: "address",
      },
    ],
    name: "deploy",
    outputs: [
      {
        internalType: "address",
        name: "",
        type: "address",
      },
    ],
    stateMutability: "nonpayable",
    type: "function",
  },
];

const _bytecode =
  "";

type UniswapV3TwapPriceOracleV2FactoryConstructorParams =
  | [signer?: Signer]
  | ConstructorParameters<typeof ContractFactory>;

const isSuperArgs = (
  xs: UniswapV3TwapPriceOracleV2FactoryConstructorParams
): xs is ConstructorParameters<typeof ContractFactory> => xs.length > 1;

export class UniswapV3TwapPriceOracleV2Factory__factory extends ContractFactory {
  constructor(...args: UniswapV3TwapPriceOracleV2FactoryConstructorParams) {
    if (isSuperArgs(args)) {
      super(...args);
    } else {
      super(_abi, _bytecode, args[0]);
    }
    this.contractName = "UniswapV3TwapPriceOracleV2Factory";
  }

  deploy(
    _logic: string,
    overrides?: Overrides & { from?: string | Promise<string> }
  ): Promise<UniswapV3TwapPriceOracleV2Factory> {
    return super.deploy(
      _logic,
      overrides || {}
    ) as Promise<UniswapV3TwapPriceOracleV2Factory>;
  }
  getDeployTransaction(
    _logic: string,
    overrides?: Overrides & { from?: string | Promise<string> }
  ): TransactionRequest {
    return super.getDeployTransaction(_logic, overrides || {});
  }
  attach(address: string): UniswapV3TwapPriceOracleV2Factory {
    return super.attach(address) as UniswapV3TwapPriceOracleV2Factory;
  }
  connect(signer: Signer): UniswapV3TwapPriceOracleV2Factory__factory {
    return super.connect(signer) as UniswapV3TwapPriceOracleV2Factory__factory;
  }
  static readonly contractName: "UniswapV3TwapPriceOracleV2Factory";
  public readonly contractName: "UniswapV3TwapPriceOracleV2Factory";
  static readonly bytecode = _bytecode;
  static readonly abi = _abi;
  static createInterface(): UniswapV3TwapPriceOracleV2FactoryInterface {
    return new utils.Interface(
      _abi
    ) as UniswapV3TwapPriceOracleV2FactoryInterface;
  }
  static connect(
    address: string,
    signerOrProvider: Signer | Provider
  ): UniswapV3TwapPriceOracleV2Factory {
    return new Contract(
      address,
      _abi,
      signerOrProvider
    ) as UniswapV3TwapPriceOracleV2Factory;
  }
}
